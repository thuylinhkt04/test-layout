...
const settings = {
  ...
  centerMode: true,
  slidesToScroll: 1,
  responsive: [settingDesktop, settingMobile],
};
  ...
  return (
    <section className="text-center my-12">
      <div className="container mb-8 lg:mb-10">
        ...
        <a
          href="/"
          className="text-blue inline-flex items-center gap-2"
          title="instagr.am/beeyoufashion"
        >
      ...
      <Slider {...settings}>
        {images.map((item, idx) => (
          <div key={idx} className="px-[6px] lg:px-[15px]">
            <div className="bg-white p-1 rounded-lg lg:rounded-2xl lg:p-2">
              <img
                className="w-full rounded-lg lg:rounded-2xl"
                src={item}
                alt="BeeYou trên Instagram"
              />
            </div>
          </div>
        ))}
